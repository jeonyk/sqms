...
<template>
    <div class="tab_contents">
        ...
        <div class="tab_area" v-show="currentTab == 0">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <th>제목</th>
                            <td><input type="text" v-model="search.mysite.reportTitle" class="form_control"></td>
                            <th>데이터기준</th>
                            <td>
            ...
            <div class="inner">
                <div class="button_box">
                    <div class="fr">
                        <button type="button" class="btn btn_md btn_outline btn_darkgray" @click="creatReport()">생성</button>
                ...
                <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th>제목</th>
                                <td>
                                    <input type="text" v-model="search.allsites.reportTitle" class="form_control w100">
                                </td>
                                <th>데이터기준</th>
                                <td>
        ...
        <!-- 보고서 생성 팝업 -->
        <ReportWikTitlePop :requestTitleParams="requestTitleParams" @responseTitleSaveForm="responseTitleSaveForm"/>

        <!-- 보고서 작성 팝업 -->
        <ReportWikDetailPop :requestParams="requestParams" @responseDtlSaveForm="responseDtlSaveForm" />
...
import { reportWikMySiteSheet, reportWikAllSitesSheet } from '../../common/data/ReportList' 
import ReportWikTitlePop from '@/pages/stats/report/ReportWikTitlePop.vue'
    ...
    export default {
        ...
        methods: {
            async getCategoryData () {
                ...
                let param = { groupCode: 'REPORT_PROGRS_STATUS' };
            ...
            creatReport() {
                // v-b-modal.ReportWrite
                this.requestTitleParams = {
                    reportTitle : '',
                    dateStr : [null, null]
                }

                this.$root.$emit('bv::show::modal', 'ReportWrite');
            },
            gridWikReportDetail() {
                this.$root.$emit('bv::show::modal', 'ReportDetailWrite');
            },
            responseTitleSaveForm(param) {  // 보고서 제목 작성완료
                // console.log("hide ReportWrite");
                this.$root.$emit('bv::hide::modal', 'ReportWrite'); // 제목 작성 닫기
                this.searchData('MY');

                // console.log("responseTitleSaveForm : %o", param);
                this.requestParams = param;
                this.gridWikReportDetail();  // 상세 팝업
            },
            responseDtlSaveForm(param) {  // 보고서 상세 완료
                // console.log(param);
                this.$root.$emit('bv::hide::modal', 'ReportDetailWrite'); // 상세화면 닫기
                this.searchData('MY');  // 재조회
            },
            initializeGrid() {
                let data     = reportWikMySiteSheet.data;
                let options  = reportWikMySiteSheet.options;
                let data2    = reportWikAllSitesSheet.data;
                let options2 = reportWikAllSitesSheet.options;
            ...
            async searchData(type) {
                ...
                if( type == 'ALL' ) {
                    url = '/sendApi/api/stats/report/reportWikAll/list';
                    ...
                    param.reportTitle  = this.search.allsites.reportTitle;
                    param.dataStartDt  = this.search.allsites.date[0];
                    param.dataEndDt    = this.search.allsites.date[1];
                    
                } else {
                    url = '/sendApi/api/stats/report/reportWikMy/list';

                    param.siteMstrId   = this.$cookies.get('siteMstrId');
                    param.progrsStatus = this.search.mysite.progrsStatus;
                    param.reportTitle  = this.search.mysite.reportTitle;
                ...
                if( type == 'ALL' ) {
                    this.grid.wikAllSitesData.loadSearchData({
                        data: data.data.datas.dlWikData,
                    });
                } else {
            ...
            searchDataClean : function(type) {
                if( type == 'ALL' ) {
                    ...
                    this.search.allsites.reportTitle  = '';
                    ...
                    this.search.mysite.reportTitle  = '';
            ...
            deleteSave : function() { // 삭제 버튼 누를경우
                ...
                let url = '/sendApi/api/stats/report/reportWik/unuse';
                let param = {
            ...
            approvalStatusChange : async function(param) {
                let url = '/sendApi/api/stats/report/reportWikAppSuccess/change';
                let params = {
                        siteMstrId : param.siteMstrId,
                        reportWikId : param.reportWikId
                };
