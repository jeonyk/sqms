<template>
   <b-modal v-bind:id="projectSelectId" title="프로젝트 선택"
      modal-class="single_page" size="xl" v-on:shown="popupOpenCheck"
      v-bind:footer-bg-variant="footerBg" static no-close-on-esc no-close-on-backdrop>
      <template #default>
            ...
            <div class="tab_contents">
               ...
               <!-- <div class="inner"> -->
                  ...
                  <div class="ib_wrap" style="padding-left:20px;padding-right:20px;padding-bottom:20px">
                     <div id="commonProjectTable" class="ibsheet_table"></div>
                  </div>
               <!-- </div> -->
            </div>
      ...
      <template #modal-footer="{}"> <!-- #modal-footer="{ ok, hide }" -->
         <b-button v-if="allBindBtn== 'Y'" class="btn btn_md btn_outline btn_darkgray" v-on:click="fn_AllChoice">전체선택</b-button>
...
<script>
   ...
   export default {
      ...
      props: {
         ...
         exceptYn : {//현재 현장정보를 제외해야하는경우(ex:타프로젝트 데이터 가져오기)
            type: String,
            default: '',
         },
      },
      ...
      methods: {
         ...
         sheetCreate: async function() { //검색 버튼 누를 경우 ibsheet-data settings
            ...
            if(this.exceptYn == 'Y'){
               param['exceptId'] = this.$store.getters['auth/getSiteMstrId'];
            }
            ...
            let cols = [
               // { Name: "SEQ", Header: "No", Type: "Int", Width: 40},
            ...
            this.heightChange(scroll,'commonProjectTable');
