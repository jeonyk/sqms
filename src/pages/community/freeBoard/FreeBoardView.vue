        ...
        <div class="lnb_container on">
            <div class="tab_contents">
                <div class="inner type2">
                    <h2 class="sub_title" v-if="viewModifiState">자유게시판 수정
                        <div class="fr">
                    ...
                    <h2 class="sub_title" v-else>자유게시판 상세
                        <div class="fr">
                        ...
                        <li v-for="(list, idx) in commentObj.list" :key="idx">
                            <div>
                                ...
                                <div class="contents">
                                    ...
                                    <template v-else>
                                        <textarea name="" id="" @change="fn_commentInputChange($event, list)" :maxLength="300" :value="list.contentsChange" class="form_control" placeholder="댓글을 입력해주세요."></textarea>
                                        <div class="count_write">
                                            <span class="index_count">{{ list.contents.length }}</span>
                                            /
                                            <span class="total_count">300</span>
                                        </div>
                                    </template>
...
<script>
   ...
   export default {
        ...
        mounted() {
            ...
            if(this.$route.params && this.$route.params.freeBoardId){
                this.freeBoardId = this.$route.params.freeBoardId;
                this.fn_view(false, '1', '1');
            }
            else{
                this.$store.dispatch('tabInfo/setSavePage',this.$route.name);
                this.$router.push({ name: 'FreeBoardSheet' })
            }
        },
        methods: {
            ...
            async fn_view(viewModifiState, addComment , viewCheck) {
                ...
                try {
                    if(this.freeBoardId){
                        ...
                        const response = await API.community.selectFreeBoardView(payload);
                        if(response.status == '200'){
                            //첨부파일 가져오기
                            let url = '/sendApi/api/file/upFileList';
                            let param = {
                                fileType : "FreeBoard",
                                fileConnectId : payload.freeBoardId,
                            };
                            ...
                            if(resultData.commentCnt > 0 && !viewModifiState){
                                ...
                                await API.community.selectBoardCommentList(payload)
            ...
            fn_change(){
                ...
                this.confirm('수정하시겠습니까?', async() => {
                    try {
                        ...
                        const response = await API.community.changeFreeBoard(payload);  
                        ...
                        if(response.status == '200' && response.data.data.rCode == "0000"){
                            ...
                            if((this.files && this.files.length > 0) || (this.deleteFiles && this.deleteFiles.length > 0)){
                                ...
                                if(this.files && this.files.length > 0){
                                    ...
                                    const subResponse = await sendFileApi(this.input.freeBoardId, "FreeBoard", this.files)
            ...
            fn_commentWrite(){
                ...
                this.confirm('등록하시겠습니까?', async() => {
                    try {
                        let payload = {
                            'freeBoardId' : this.input.freeBoardId,
                            'contents' : this.commentObj.input
                        };

                        const response = await API.community.writeBoardComment(payload);
                        if(response.status == '200' && response.data.data.rCode == "0000"){
                            this.alert('등록되었습니다.');
            ...
            fn_changeComment(obj){
                ...
                this.confirm('수정하시겠습니까?', async() => {
                    try {
                        let payload = {
                            'boardCommentId' : obj.boardCommentId,
                            'contents' : obj.contentsChange,
                        };

                        const response = await API.community.changeBoardComment(payload);
                        if(response.status == '200' && response.data.data.rCode == "0000"){
                            this.alert('수정되었습니다.');
                            this.fn_view(this.viewModifiState);
                        }
            ...
            fn_deleteComment(obj){
                this.confirm('삭제하시겠습니까?', async() => {
                    try {
                        ...
                        const response = await API.community.deleteBoardComment(payload);
                        if(response.status == '200' && response.data.data.rCode == "0000"){
                            this.alert('삭제되었습니다.');
                            this.fn_view(this.viewModifiState);
                        }
