            ...
            fn_write(){
                ...
                this.confirm('등록하시겠습니까?', async() => {
                    try {
                        ...
                        const response = await API.community.writeFreeBoard(payload);  
                        
                        if(response.status == '200' && response.data.data.rCode == "0000"){
                            //파일 추가
                            if(response.data.data.freeBoardId != ''){

                                if(this.files && this.files.length > 0){
                                    requestFileOptions.headers['Authorization'] = this.$cookies.get("Authorization")
                                    const subResponse = await sendFileApi(response.data.data.freeBoardId, "FreeBoard", this.files)
