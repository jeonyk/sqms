    ...
    <div>
        <div class="lookup_box multi_line">
            ...
            <caption class="sr_only"></caption>
                ...
                <tbody>
                    <tr>
                        <th>협력사</th>
                        <td><input type="text" class="form_control size_md" v-model="search.partnerName" placeholder="입력해주세요"></td>              
                        <th>기간</th>
                        <td>
                            <DatePickerRange ref="datePicker_range" v-model="search.workerDt"/>
                        </td>
                        <th></th>
                        <td></td>
                        <td class="ar">
        ...
        <div class="inner">
            ...
            <sqms-grid
                ...
                @click="clickgridData"
                @init="initSheet"
                ref="workerEduStatusAllSiteRef"
                :excelFileName="`신규_근로자_교육_현황_${$moment().format('YYYYMMDD')}.xlsx`"
                downloadable
            >  
            </sqms-grid>
        </div>
        <!-- 교육 현황 상세 팝업 -->
        <b-modal id="EducationStatusDetail2" title="교육 현황 상세" 
...
<script>
   import { EducationStatusList2 } from '@/pages/common/data/InspectionList'
   import { EducationStatusDetail, EducationStatusDetail2 } from '@/pages/common/data/PopupList'
   ...
   export default {
        ...
        async created() {
            ...
            this.columns = EducationStatusList2.options.Cols
        ...
        methods: {
            ...
            async searchDataDtl (isReset) {
                ...
                return await this.getEduCatorCompleteList(this.seachDtl)
            },
            async clickgridData (evt) {
                // const rowData = evt.sheet.getRowValue(evt.row)
                if(evt.col === 'detailBtn'){
                    this.$bvModal.show('EducationStatusDetail2')
            ...
            async getEduCationStatusList(payload = this.search) {
                ...
                try {
                    ...
                    if(response?.dlCurrentState){
                        this.gridData = response.dlCurrentState
